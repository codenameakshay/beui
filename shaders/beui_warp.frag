...
// Port of @paper-design/shaders `warpFragmentShader` (animated, texture).
// Deviations (shared — see beui_voronoi.frag): v_patternUV inlined; MAX_COLORS 8
// (source 10); fwidth() -> analytic patternUV AA; randomG inlined; banding fix
// uses FlutterFragCoord.
...
uniform float u_proportion;        // 38
uniform float u_softness;          // 39
uniform float u_shape;             // 40
uniform float u_shapeScale;        // 41
uniform float u_distortion;        // 42
uniform float u_swirl;             // 43
uniform float u_swirlIterations;   // 44
...
void main() {
  ...
  float aa = max(2.0 * pxUV, 1e-4); // fwidth(shape) -> analytic
  for (int i = 1; i < MAX_COLORS; i++) {
    if (i >= int(u_colorsCount)) break;
    float m = clamp(mixer - float(i - 1), 0.0, 1.0);
    float localMixerStart = floor(m);
    float softness = .5 * u_softness + aa; // fwidth(m) -> analytic
