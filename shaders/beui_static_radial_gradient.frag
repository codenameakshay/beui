...
// Port of @paper-design/shaders `staticRadialGradientFragmentShader` (static).
// Deviations (shared — see beui_mesh_gradient.frag): v_objectUV inlined;
// MAX_COLORS capped at 8 (source 10); fwidth(mLinear) -> analytic objectUV
// per-pixel width; helpers inlined.
...
uniform float u_radius;            // 42
uniform float u_focalDistance;     // 43
uniform float u_focalAngle;        // 44
uniform float u_falloff;           // 45
uniform float u_mixing;            // 46
uniform float u_distortion;        // 47
uniform float u_distortionShift;   // 48
uniform float u_distortionFreq;    // 49
uniform float u_grainMixer;        // 50
uniform float u_grainOverlay;      // 51
...
void main() {
  ...
  for (int i = 1; i < MAX_COLORS + 1; i++) {
    ...
    float aa = max(aaPx * u_colorsCount, 1e-4); // fwidth(mLinear) -> analytic
    float width = min(u_mixing, 0.5);
    float tt = clamp((mLinear - (0.5 - width - aa)) / (2. * width + 2. * aa), 0., 1.);
    float p = mix(2., 1., clamp((u_mixing - 0.5) * 2., 0., 1.));
    float m = tt < 0.5 ? 0.5 * pow(2. * tt, p) : 1. - 0.5 * pow(2. * (1. - tt), p);
    float quadBlend = clamp((u_mixing - 0.5) * 2., 0., 1.);
    m = mix(m, m * m, 0.5 * quadBlend);
    if (i == 1) outerShape = m;
    vec4 col = u_colors[i - 1];
    col.rgb *= col.a;
    gradient = mix(gradient, col, m);
  }
