...
// Port of @paper-design/shaders `smokeRingFragmentShader` (animated, texture).
// Deviations (shared — see beui_voronoi.frag): v_objectUV inlined; MAX_COLORS 8
// (source 10); dynamic u_colors[idxLast] -> constant-index loop; the gradient
// loop is guarded to active colors (our padding is transparent, not repeated);
// randomR inlined; banding fix uses FlutterFragCoord.

#define TWO_PI 6.28318530718
#define PI 3.14159265358979323846
#define MAX_COLORS 8
#define MAX_NOISE_ITER 8
...
uniform float u_thickness;         // 42
uniform float u_radius;            // 43
uniform float u_innerShape;        // 44
uniform float u_noiseScale;        // 45
uniform float u_noiseIterations;   // 46
...
void main() {
  ...
  for (int k = 0; k < MAX_COLORS; k++) {
    if (k == idxLast) gradient = u_colors[k];
  }
  gradient.rgb *= gradient.a;
  for (int i = MAX_COLORS - 2; i >= 0; i--) {
    if (i >= idxLast) continue; // only active colors (padding is transparent)
