...
/// One cell of a [BeuiCreateMenu] grid (source `MenuItem`).
@immutable
class BeuiCreateMenuItem {
  /// Creates a menu item. Provide [icon] or [customIcon].
  const BeuiCreateMenuItem({required this.label, this.icon, this.customIcon})
    : assert(
        icon != null || customIcon != null,
        'Provide an icon glyph or a custom icon widget.',
      );

  /// Cell label; also the [BeuiCreateMenu.onSelect] payload.
  final String label;
...
/// The source's default `ITEMS` (Lucide glyphs, overridable per menu).
const beuiDefaultCreateMenuItems = [
  BeuiCreateMenuItem(label: 'Doc', icon: LucideIcons.file_text),
  BeuiCreateMenuItem(label: 'Board', icon: LucideIcons.layout_grid),
  BeuiCreateMenuItem(label: 'Table', icon: LucideIcons.table),
  BeuiCreateMenuItem(label: 'Folder', icon: LucideIcons.folder_closed),
  BeuiCreateMenuItem(label: 'Reminder', icon: LucideIcons.bell),
  BeuiCreateMenuItem(label: 'Link', icon: LucideIcons.link),
];

/// Folder-open feel: a touch of overshoot as the panel expands, kept subtle
/// (source `SPRING_FOLDER`, 300 · 32 · 0.9).
const _folderSpring = SpringMotion(
  SpringDescription(mass: 0.9, stiffness: 300, damping: 32),
);

/// Per-item bloom spring (source item transition, 440 · 34).
const _itemSpring = SpringMotion(
  SpringDescription(mass: 1, stiffness: 440, damping: 34),
);

const _triggerSize = Size(144, 44); // w-36 h-11
const _panelMaxWidth = 420.0; // min(86vw, 420px)
const _enterMs = 800.0; // hosts the 0.08s + 0.45s iris + item staggers

/// A "Create" button that blooms open into a grid menu — the Flutter port of
/// beUI's create-menu (upstream slug `bloom-menu`).
///
/// The box morphs from the trigger to the panel on the folder spring, growing
/// from its center outward; the grid irises open (`inset(45% 34%)` → 0, 450ms
/// `EASE_OUT`) while cells bloom in with a radial stagger (distance from the
/// grid center × 70ms, 440/34 spring, scale 0.85 + 6px blur). Esc and outside
/// taps close it.
///
/// Rendered through [BeuiOverlay] (transparent barrier): the panel escapes
/// ancestor clips and stays tappable beyond the trigger's bounds — the
/// containing-block-safe version of the source's absolute overlay.
///
/// Reduced motion swaps trigger/panel with quick fades — no morph, iris, or
/// item movement.
class BeuiCreateMenu extends StatefulWidget {
  /// Creates a create-menu.
  const BeuiCreateMenu({
    this.items = beuiDefaultCreateMenuItems,
    this.onSelect,
    this.triggerLabel = 'Create',
    super.key,
  });

  /// The grid cells (3 columns).
  final List<BeuiCreateMenuItem> items;

  /// Fires with the chosen cell's label; the menu closes after.
  ...
  @override
  State<BeuiCreateMenu> createState() => _BeuiCreateMenuState();
}

class _BeuiCreateMenuState extends State<BeuiCreateMenu>
    with SingleTickerProviderStateMixin {
  bool _open = false;

  /// True while the overlay is mounted (open or morphing closed) — the
  /// in-tree trigger stays hidden until the box has shrunk back onto it.
  ...
  /// 80–530ms, radial item staggers) — the same pattern as text-reveal.
  ...
  void _scheduleMeasure() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      if (box.size != _panelSize) setState(() => _panelSize = box.size);
    });
  }
  ...
  Widget _buildOverlay(
    ...
    return CompositedTransformFollower(
      ...
      child: MotionBuilder<Size>(
        value: _open ? panelSize : _triggerSize,
        from: _triggerSize,
        ...
        child: Stack(
          ...
          children: [
            ...
            // The trigger label rides the shrinking box on close, so the
            // morph lands seamlessly on the real (unhidden) trigger.
            if (!_open)
              IgnorePointer(
...
class _TriggerState extends State<_Trigger> {
  ...
  Widget build(BuildContext context) {
    ...
    body = SingleMotionBuilder(
      value: _pressed && !reduce ? 0.97 : 1.0, // whileTap
