...
///   [beuiSpringSwap] token, released staggered left-to-right, via
///   [BeuiCascadeText] (also used by `StatefulButton`). Non-text content
///   (icons) and reduced motion fall back to [roll], matching the source.
...
class BeuiActionSwapText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (variant == BeuiActionSwapVariant.cascade && !reduce) {
      return _AnimatedWidth(child: BeuiCascadeText(text, style: style));
    }
...
class _BeuiActionSwapButtonState extends State<BeuiActionSwapButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final palette = buttonPalette(widget.variant, colors, _hovered);
